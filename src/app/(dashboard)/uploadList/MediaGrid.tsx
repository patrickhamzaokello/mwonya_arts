...
class MediaCard extends React.Component<{ upload: any, onDelete: any }> {
    render() {
        let {upload, onDelete} = this.props;
        return (
            <div key={upload.id}
                 className="bg-white rounded-lg shadow-sm overflow-hidden p-4 flex items-center space-x-4">
                {upload.type === 'image' && (
...
class MediaGrid extends React.Component<{ error: any, mediaUploads: any, onDelete: any }> {
    render() {
        let {error, mediaUploads, onDelete} = this.props;
        return (
            <div>
                {error && (
                    <div
                        className={`p-4 mb-4 rounded-lg ${error.success ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                        {error.message}
                    </div>
                )} {
                <div className="grid grid-cols-1 gap-4">
                    {mediaUploads.length === 0 ? (
                        ...
                        mediaUploads.map((upload: { id: React.Key | null | undefined; }) => (
